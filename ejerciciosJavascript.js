...
//for Each()
/*
const listGenres = (genreArray) => {
    let result = [];
    let listValue = genreArray.forEach((genero) => {
        if (!result.includes(genreArray.genero)) {
            result = [...result, genero.genre];
        }
    });
    return result;
}

console.log(listGenres(movies));

const listGenre = (genreArray) => {
    let totalGenre =
        let listValue = genreArray.filter((genero) => {
            return genero.genre === genre;
        });

    if (!listValue) {
        return `No existen peliculas de ese genero ${genre}`;
    } else {
        return listValue;
    }
};

console.log(listGenre(movies))
...
const califiaciones = (movies) => {
    let listaCalificaciones = movies.reduce((object, movie) => {
        if (!object.includes(movie.rating)) {
            return object = [...object, movie.rating]
        } return object
    }, []);
...
const listDePeliculas = (moviesArray, rating) => {
    let resultPeliculas = moviesArray.reduce((object, movies) => {
...
console.log(listDePeliculas(movies, "R"))
console.log(listDePeliculas(movies, "PG"))
